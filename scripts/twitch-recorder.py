    ...
    def loop_check(self, recorded_path, processed_path):
        while True:
            ...
            if status == TwitchResponseStatus.NOT_FOUND:
                ...
                completeP = subprocess.run(["streamlink", "--twitch-disable-ads", "twitch.tv/" + self.username, self.quality, "--stream-url"], capture_output=True, text=True)
                if completeP.stdout:
                  url = completeP.stdout.rstrip("\n")
                  subprocess.call(["/usr/local/bin/alexa_ffmpeg_twitch.sh", url, "icecast://*****:******@icecast.tuxe.es:8000/recalbox.mp3"])
